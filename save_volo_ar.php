<?php
    ...
    function Volo_a() {
        ...
        $codvolo2 = mysqli_real_escape_string($conn, $_POST['codvolo2']);
        $durataminuti2 = mysqli_real_escape_string($conn, $_POST['durata_minuti2']);
        $orario_partenza2 = mysqli_real_escape_string($conn, $_POST['orario_partenza2']);
        $orario_arrivo2 = mysqli_real_escape_string($conn, $_POST['orario_arrivo2']);
        $delta2 = mysqli_real_escape_string($conn, $_POST['delta2']);
       ...
       // verifica se l'hotel è già presente per l'utente, in quella specifica città e per quella specifica data di partenza
        ...
        $res = mysqli_query($conn, $query) or die(mysqli_error($conn));
        // se il è già presente lo sostituisco
        if(mysqli_num_rows($res) > 0) {
            $query = "UPDATE voli SET  content = JSON_OBJECT('id', '$id', 'partenza', '$partenza', 'destinazione', '$destinazione', 'origin_aeroport', '$origin_aeroport','destination_aeroport', '$destination_aeroport', 'prezzo', '$prezzo',
             ...
             'orario_arrivo', '$orario_arrivo',  'delta', '$delta', 'codvolo2', '$codvolo2','durata_minuti2', '$durataminuti2','orario_partenza2', '$orario_partenza2',
             'orario_arrivo2', '$orario_arrivo2',  'delta2', '$delta2','tipo', '$tipo')
        ...
        'orario_arrivo', '$orario_arrivo', 'scali', '$scali', 'codvolo', '$codvolo', 'delta', '$delta',
        'codvolo2', '$codvolo2','durata_minuti2', '$durataminuti2','orario_partenza2', '$orario_partenza2',
        'orario_arrivo2', '$orario_arrivo2',  'delta2', '$delta2', 'tipo', '$tipo'))";
        error_log($query);
        ...
        if(mysqli_query($conn, $query) or die(mysqli_error($conn))) {
            ...
            $res = mysqli_query($conn, $query) or die(mysqli_error($conn));
            if(mysqli_num_rows($res) > 0) {
                // Viaggio già presente, aggiorna il volo_id
